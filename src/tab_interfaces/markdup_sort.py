...
class MarkDuplicatesSpark(Interface):
    ...
    def create_input_panel(self, tab):
        ...
        process_button = ttk.Button(input_panel, text="Process", 
                            command=lambda: self.process_data(input_entry, output_entry, result_text))
        ...
        export_frame.grid(row=4, column=0, columnspan=2, pady=10, sticky=tk.E)
    ...
    def process_data(self, input_entry, output_entry, result_text):
        input = input_entry.get()
        output = output_entry.get()
        
        result = f"gatk MarkDuplicatesSpark -I {input} -O {output}"
